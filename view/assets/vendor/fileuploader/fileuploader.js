...
(function($) {
				...
				f = {
					...
					thumbnails: {
                        ...
                        popup: function(item) {
                            ...
                            n.thumbnails.popup.onShow && $.isFunction(n.thumbnails.popup.onShow) ? n.thumbnails.popup.onShow(item, l, p, o, s, 'weqweqw') : null;
	...
	$.fn.fileuploader.defaults = {
                ...
                template: function(data) { return '<div class="fileuploader-popup">' +
                    ...
                    '<div class="fileuploader-popup-content">' +
                        '<ul class="fileuploader-popup-meta">' +
                            '<li>' +
                                '<span>Nome:</span>' +
                                '<h5 class="custom_name_change">${name}</h5>' +
                            '</li>' +
                            (data.format != 'image' ? '<li>' +
                                '<span>Nome Customizado:</span>' +
                                '<form action="" method="POST" id="custom_name">' +
	                                '<div class="input-group custom-name">' +
                                		'<input type="hidden" name="item" id="custom_name_item" value="">' +
	                                	'<input type="text" name="custom_name" class="form-control" value="${name}">' +
										'<div class="input-group-append">' +
									    	'<button class="btn btn-primary" type="submit" title="SALVAR">SALVAR</button>' +
										'</div>' +
									'</div>' +
								'</form>' +
								'<script>$("#custom_name").submit(function(e){e.preventDefault();var formData = new FormData(this);$.ajax({url: "assets/data/produtos/ajax_custom_name.php",data: formData,type: "post",success: function(data){var sucesso = data.split(\''+'|'+'\'); ChangeNameView(sucesso[1],sucesso[0])},processData: false,cache: false,contentType: false});});</script>'+
                            '</li>' : ''
                            ) +
                            '<li>' +
                                '<span>Tipo:</span>' +
                    	...
                    	'<ul class="fileuploader-popup-tools ' + (data.format != 'image' ? 'd-none' : 'd-flex justify-content-between' ) + '">' +
                        ...
                        '</ul>' +
                        '<div class="buttons">' +
                        	'' + (data.format != 'image' ? '<a href="#" class="fileuploader-popup-button btn btn-default btn-block" data-action="cancel">FECHAR</a>' : '<a href="#" class="fileuploader-popup-button btn btn-default" data-action="cancel">CANCELAR</a>' ) + ''+
                            '<a href="#" class="fileuploader-popup-button btn btn-primary pull-right ' + (data.format != 'image' ? 'd-none' : '' ) + '" data-action="save">CONFIRMAR</a>' +
                        '</div>' +
                        '<div class="aviso-editor text-center mt-5 text-muted ' + (data.format != 'image' ? 'd-none' : '' ) + '">' +
                        	'Ao editar uma foto atualize seu navegador apertando Ctrl + F5 ou Command + R para limpar o cache de seu navegador.' +
                        '</div>' +
        ...
        captions: {
            ...
            errors: {
                filesLimit: 'Apenas arquivos ${limit} podem ser carregados.',
                filesType: 'Apenas arquivos ${extensions} podem ser carregados.',
